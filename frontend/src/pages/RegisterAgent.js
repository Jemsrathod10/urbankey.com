...
import { Briefcase, Mail, Lock, User, Phone, Building, CheckCircle, MapPin, AlertCircle, ArrowRight, Eye, EyeOff, Clock } from 'lucide-react';
...
const RegisterAgent = () => {
  const [formData, setFormData] = useState({
    ...
    companyName: '', address: '', termsAccepted: false
  });
  ...
  const validateForm = () => {
    ...
    if (formData.companyName.trim().length < 2) return "Company name is required";
    if (formData.address.trim().length < 5) return "Please provide a more detailed office address";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await API.post('/auth/register', {
        ...formData,
        role: 'agent',
        businessInfo: { companyName: formData.companyName, address: formData.address }
      });
      if (data.needsApproval || data.success) setSuccess(true);
    } catch (err) {
    ...
    return (
      <div style={styles.container}>
        <div style={styles.successCard} className="premium-card">
          ...
          <h2 style={styles.title}>Application Pending</h2>
          <div style={styles.statusBadge}>UNDER REVIEW</div>
          <p style={styles.successSubtext}>
            Thank you for applying as a <b>Professional Agent</b>. Your account is currently 
            awaiting manual verification by our <b>Administrator</b>. 
            <br /><br />
            Once your credentials are approved, we will send a confirmation email to 
            access your dashboard.
          </p>
          <button onClick={() => navigate('/login')} style={styles.primaryBtn}>Return to Login</button>
        </div>
      </div>
    );
  }
    ...
    <div style={styles.container}>
      ...
      <div style={styles.card} className="premium-card">
        <div style={styles.headerSection}>
          <div style={styles.logoIcon}><Briefcase size={28} color="#fff" /></div>
          <h2 style={styles.title}>Agent Partner</h2>
          <p style={styles.subtitle}>List professional inventories and manage leads globally.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <div style={styles.row}>
            <div style={styles.inputGroup}>
              ...
              <div style={styles.inputWrapper}>
                ...
                <input style={styles.input} className="input-focus" type="email" placeholder="agent@email.com" required onChange={e => setFormData({...formData, email: e.target.value})} />
              </div>
            </div>
            <div style={styles.inputGroup}>
          ...
          <div style={styles.inputGroup}>
            <label style={styles.label}>Agency / Company Name</label>
            <div style={styles.inputWrapper}>
              <Building size={18} style={styles.innerIcon} />
              <input style={styles.input} className="input-focus" type="text" placeholder="Dream Homes Realty" required onChange={e => setFormData({...formData, companyName: e.target.value})} />
            </div>
          </div>
          ...
          <div style={styles.inputGroup}>
            ...
            <div style={styles.inputWrapper}>
              ...
              <input style={styles.input} className="input-focus" type="text" placeholder="Corporate Suite, Business Bay" required onChange={e => setFormData({...formData, address: e.target.value})} />
            </div>
          </div>
          ...
          <div style={styles.termsBox}>
            <input type="checkbox" id="terms" style={styles.checkbox} checked={formData.termsAccepted} onChange={e => setFormData({...formData, termsAccepted: e.target.checked})} />
            <label htmlFor="terms" style={styles.checkboxLabel}>
              I agree to the <span onClick={() => setIsModalOpen(true)} style={styles.linkSpan}>Professional Terms</span>.
            </label>
          </div>
        ...
        <p style={styles.footerText}>Already part of UrbanKey? <Link to="/login" style={styles.link}>Login</Link></p>
      </div>
